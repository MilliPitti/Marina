...
//                    result_bioMass_i -= + (koeffmat[j][1] * astx * dbmdx + koeffmat[j][2] * asty * dbmdy) * eleCurrentData.wlambda;
                    ...
                    for (int l = 0; l < 3; l++) {
                        ...
                        final double gl = (l == j) ? 1. :  Math.min(CurrentModel2DData.extract(ele.getDOF(l)).wlambda, CurrentModel2DData.extract(ele.getDOF(l)).totaldepth/Math.max(CurrentModel2D.WATT,cmd.totaldepth));
