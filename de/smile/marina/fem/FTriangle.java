...
//        elemFormParameter = Math.pow( Math.tanh(xi*si*3.0)/Math.tanh(3.0) , 3. ); // Christoph Original
////        elemFormParameter = Math.tanh(xi*si*3.0)/Math.tanh(3.0); // Peters Anpassung
// Heller-Milbradt
//        final double xi = area /(dofs[0].distance(dofs[1])*dofs[0].distance(dofs[1])+dofs[1].distance(dofs[2])*dofs[1].distance(dofs[2])+dofs[2].distance(dofs[0])*dofs[2].distance(dofs[0])) / 0.144338;
//        elemFormParameter = 1.-Math.tanh((xi-1.)/20.);
    ...
    public final double getVectorSize(double vx,double vy){
		...
		double p1y = dofs[i].y+vy;
                ...
                double s = (dofs[i].x * (dofs[i1].y - dofs[i2].y) + dofs[i1].x * (dofs[i2].y - dofs[i].y) +
                		dofs[i2].x * (dofs[i].y - dofs[i1].y)) / (dofs[i].x * (dofs[i1].y - dofs[i2].y) +
                ...
                double t = (dofs[i].x * (p1y - dofs[i1].y) + p1x * (dofs[i1].y - dofs[i].y) +
                		dofs[i1].x * (dofs[i].y - p1y)) / (dofs[i].x * (dofs[i2].y - dofs[i1].y) +
...
//    	return mat;
    ...
    public final boolean contains(Point3d p) {
        ...
        double a2 = (dofs[1].x-dofs[0].x)*(dofs[2].y-dofs[0].y)-(dofs[2].x-dofs[0].x)*(dofs[1].y-dofs[0].y); // vorzeichenbehafteter Flaecheninhalt
        // erste Teilflaeche
