 ...
 * @author milbradt
 */
public class ShepardInterpolatedMeteorologicalModel2D extends TimeDependentFEApproximation implements FEModel, TimeDependentModel {
    ...
    public ShepardInterpolatedMeteorologicalModel2D(FEDecomposition fe, OKWind[] windtimeseries, MeteorologicalDat dat) {
        fenet = fe;
        femodel = this;
...
//        System.out.println(time+" "+data.insolation+ " "+I_n);
    ...
    class ShepardMeteorologyData2D extends MeteorologyData2D {
        ...
        double w[];         // [0..n]-Vektor mit den Gewichten fuer die Stuetzwerte; haengt ab von r und mue.

        public ShepardMeteorologyData2D(DOF dof) {
            super();
            double[] r = new double[windtimeseries.length]; // [0..n]-Vektor mit den quadratischen  Euklidischen Abstaenden der Stuetzstellen von der Interpolationsstelle
            int j;
