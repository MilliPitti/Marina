...
public class BedLoad2DvanRijn2007 implements BedLoad2DFormulation {
    ...
    public double[] getLoadVector(DOF dof) {
        ...
        if (cmd.totaldepth > CurrentModel2D.halfWATT) {
            ...
            final double lambda = Math.min(1., Math.max(0.,smd.zh - smd.z)/smd.bound); // increasing factor depending on not erodible bottom
            // Bodenneigung
            ...
            final double factor = Math.max(0.,smd.innerFrictionAngle-(smd.bottomslope -1.))/smd.innerFrictionAngle*lambda + (1.-lambda);
            ...
            if (u_diff > 1.E-5) {
                ...
                smd.bedload = Math.min(1. / SedimentModel2D.morphFactor * Math.max(((smd.bottomslope - 1.) > smd.innerFrictionAngle) ? 1. : 0., cmd.cv) * Math.max(0., smd.zh - smd.z) * (1. - smd.porosity), smd.bedload);
