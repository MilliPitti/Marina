...
public class GroundWaterModel2D extends TimeDependentFEApproximation implements FEModel, TicadModel {
    ...
    public GroundWaterModel2D(FEDecomposition fe, GroundWaterDat _groundwaterdata) {
        ...
        Enumeration<BoundaryCondition> be = bh.elements();
        ...
        if (groundwaterdat.upperImpermeabilityThickness_name != null) {
            ...
            for (DOF dof : fenet.getDOFs()) {
                GroundWater2DData
                        .extract(dof).upperImpermeableLayer = groundwaterdat.standartUpperImpermeabilityThickness;
            }
        }
    ...
    public double[] initialSolution(double time) {
        ...
        for (int ii = 0; ii < NumberOfThreads; ii++) {
            iloop[ii] = new initalSolutionLoop(anzdofs * ii / NumberOfThreads, anzdofs * (ii + 1) / NumberOfThreads, x,
                    time);
            iloop[ii].start();
        }
        for (int ii = 0; ii < NumberOfThreads; ii++)
    ...
    public double[] getRateofChange(double time, double x[]) {
        ...
        for (DOF dof : fenet.getDOFs()) {
            ...
            result[H + i] *= 3. / gamma;
            double rh = (3. * result[H + i] - dof_data[i].dhdt) / 2.; // zusaetzlichen Stabilisierung in Anlehnung am
                                                                      // expliziten Adams-Bashford 2. Ordnung
                ...
                for (int j = 0; j < 3; j++)
                    if (m[j] >= WATT) {
                        ...
                        if ((h[j] < h[(j + 1) % 3]) || (h[j] < h[(j + 2) % 3])) {
                            ...
                            hdx += ((1. - m[j] / WATT) * 0.5 * (h[(j + 1) % 3] + h[(j + 2) % 3]) + m[j] / WATT * h[j])
                                    * koeffmat[j][1];
                            hdy += ((1. - m[j] / WATT) * 0.5 * (h[(j + 1) % 3] + h[(j + 2) % 3]) + m[j] / WATT * h[j])
                                    * koeffmat[j][2];
                        }
                    }
            } else if (iwatt == 2) {
    ...
    public void setBoundaryCondition(DOF dof, double t) {
        ...
        if (current2d != null) {
            if ((current2d.z + current2d.eta > CurrentModel2D.WATT)) {
                if ((current2d.eta - x[H + i]) > 0.)
                    x[H + i] = current2d.eta;
                // if ((current2d.eta-x[H+i])>0.) groundwaterdata.source =
                // Function.max(0,current2d.eta-x[H+i]);
            }
        }

        if ((x[H + i] + dof_data[i].zG) <= 0.)
            x[H + i] = -dof_data[i].zG;
            ...
            while (p_count < (rand_knoten + gebiets_knoten)) {
                ...
                if (!line.startsWith("C")) {
                    ...
                    if (Double.isNaN(kf) || kf < 0) {
                        ...
                        System.out.println(
                                "********************************       ERROR         ***********************************");
                        System.out.println("Invalid kf-value (kf=NaN or kf<0.0) in Permeability-File: <" + filename
                                + "> node number <" + p_count + ">");
    ...
    private void readPermeabilityFromJanetBin(String filename) {
        ...
        try {
            ...
            if (anzk == fenet.getNumberofDOFs()) {
                ...
                System.out.println("system und permeability.jbf different number of nodes");
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("Fehler beim Lesen der Permeabilitaets-Datei!");
            System.exit(0);
        ...
        try {
            ...
            while (p_count < (rand_knoten + gebiets_knoten)) {
                ...
                if (!line.startsWith("C")) {
                    ...
                    // ***********************************");
                    // System.out.println("Invalid zG-value (zG=NaN or zG<0.0) in
                    // Impermeability-File: <"+filename+"> node number <"+p_count+">");
                    // System.out.println("To correct this problem ensure that node nr <"+p_count+">
                    // has a correct floating point (greater zero)");
    ...
    private void readImpermeabilityFromJanetBin(String filename) {
        ...
        try {
            ...
            if (anzk == fenet.getNumberofDOFs()) {
                ...
                System.out.println("system und impermeability.jbf different number of nodes");
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("Fehler beim Lesen der Impermeabilitaets-Datei!");
            System.exit(0);
    ...
    // *****************
    /**
     * the method readSurfaceWaterFromSysDat read a stationary free surface water
     * level as boundary condition
     * from a sysdat-file named filename
     * 
     * @param nam name of the file to be open
     */
    ...
    private void readSurfaceWaterFromSysDat(String filename) {
        ...
        try {
            ...
            System.out.println(
                    "\tReading stationary free surface water level as boundary condition from (in TiCAD-System.Dat-Format): "
                            + filename);
            ...
            while (p_count < (rand_knoten + gebiets_knoten)) {
                ...
                if (!line.startsWith("C")) {
                    ...
                    if (Double.isNaN(swl)) {
                        ...
                        System.out.println(
                                "********************************       ERROR         ***********************************");
                        System.out.println("Invalid surface water level-value (swl=NaN) in SurfaceWater-File: <"
    ...
    /**
     * the method readSurfaceWaterFromJanetBin read a stationary free surface water
     * level as boundary condition
     * from a JanetBinary-file named filename
     * 
     * @param nam name of the file to be open
    ...
    private void readSurfaceWaterFromJanetBin(String filename) {
        ...
        try {
            ...
            if (anzk == fenet.getNumberofDOFs()) {
                ...
                System.out.println("system und surfacewater.jbf different number of nodes");
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("Fehler beim Lesen der SurfaceWater-Datei!");
            System.exit(0);
    ...
    /**
     * the method readImpermeableLayer read the datas for the lower Impermeable
     * Layer - the Ground of the aquifer
        ...
        try {
            ...
            try (BufferedReader r = new BufferedReader(new InputStreamReader(is))) {
                ...
                if (anzk == fenet.getNumberofDOFs()) {
                    ...
                    System.out.println("system.dat und imperLayer.dat different number of nodes");
            }

        } catch (Exception e) {
            System.out.println("cannot open file: " + nam);
            System.out.println(
                    "use standard value for impermeable Layer: " + GroundWater2DData.extract(fenet.getDOF(0)).zG);
        }
    }

    /**
     * the method readOverlayingStrata read the Thickness for the uppere Impermeable
     * Layer - for strained aquifer
     * from a file named nam
     *
     * @param nam name of the file to be open
     */
        ...
        try {
            ...
            try (BufferedReader r = new BufferedReader(new InputStreamReader(is))) {
                ...
                if (anzk == fenet.getNumberofDOFs()) {
                    ...
                    System.out.println("system.dat und upperimperLayer.dat different number of nodes");
            }
    ...
    private void readOverlayingStrataFromJanetBin(String filename) {
        ...
        try {
            ...
            if (anzk == fenet.getNumberofDOFs()) {
                ...
                System.out.println("system und upperImperLayer.jbf different number of nodes");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public final void readBoundCond() {
        ...
        try {
            for (BoundaryCondition bc : groundwaterdat.rndwerteReader
                    ...
                    switch (Art_Rndwrt) {
                        case TicadIO.H_GESETZT:
                            ...
                            bh.addElement(new BoundaryConditionOld(KnotenNr[K], boundcond)
                                    .getBoundaryCondition(BoundaryCondition.groundwater_free_surface));
                            break;
                    }
                }
