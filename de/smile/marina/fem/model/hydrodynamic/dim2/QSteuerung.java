...
/* START: bestimmen der mittleren Wasserspiegellage ueber den Querschnitt */            
            ...
            for ( int i = 0; i < knotennummern.length; i++ ){
                ...
                SurfaceWaterModelData cdata = SurfaceWaterModelData.extract(dof); // Peter 20.03.25 // Instabil am Q Einlaufrand
                if (cdata.totaldepth > WATT) {
                    h_mean += cdata.eta;
                    n++;
                }
//                for (FElement elem : dof.getFElements()) { // Peter 20.03.25 auskommentiert // Instabil am Q Einlaufrand
//                    for (int ll = 0; ll < 3; ll++) {
//                        if (elem.getDOF(ll) == dof) {
//                            for (int ii = 0; ii < 3; ii++) {
//                                int jtmp = elem.getDOF((ll + ii) % 3).number;
//                                SurfaceWaterModelData cdata = SurfaceWaterModelData.extract(sysdat.getDOF(jtmp));
//                                if (cdata.totaldepth > WATT) {
//                                    h_mean += cdata.eta;
//                                    n++;
//                                }
...
//                }
            ...
            if(n>0){   /** todo hier gibt es noch Optimierungspotential */
                h_mean /= n;
//                h_mean = Function.max(h_mean,zmin+Function.max(WATT,Math.abs(qzeitreihe.getValue(t))/(Math.sqrt(PhysicalParameters.G * 20 *WATT)*s))); // 20.05.2025 auskommentiert 
            }else{ // alle Knoten der RB haben Tiefe = 0, falls Q != 0 so muss der wasserspiegel angehoben werden!!
                h_mean = zmin+Function.max(WATT,Math.abs(qzeitreihe.getValue(t))/(Math.sqrt(PhysicalParameters.G * 10 *WATT)*s));
            }
/* END: bestimmen der mittleren Wasserspiegellage ueber den Querschnitt */
    ...
    // Funktion zum iterativen Berechnen der Geschwindigkeitsverteilung des uebergebenen QS
...
//        System.out.println("Flaeche = "+A);
        ...
        if(Math.abs(Q)>epsilon){
            // Iteratives Berechnen der Geschwindigkeitsverteilung, solange bis Qvorh vom Eingangswert
...
//            System.out.println(Q + "  " + berechneQvorh(v));
    ...
    // Funktion zum Berechnen einer Geschwindigkeitsverteilung (abhaengig vom uebergebenen Qein)
    ...
    private void ausgabe() {
        ...
        int anz = v.getSizeOfValues();
        for (int i=0; i<anz; i++) {
            System.out.println(knotennummern[i]+" , "+v.getValueAt(i)[0]+" , "+v.getValueAt(i)[1]+" + "+d.getValueAt(i)[1]);
        }
        System.out.println("");
    }
