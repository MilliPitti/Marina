...
public class CurrentModel1D extends TimeDependentFEApproximation implements FEModel {
    ...
    public void setBoundaryCondition(DOF dof,double t) {
        if(dof.number==0) {CurrentModel1DData cmd = CurrentModel1DData.extract(dof); cmd.u=0.;}
        if(dof.number==numberofdofs-1) {CurrentModel1DData cmd = CurrentModel1DData.extract(dof); cmd.u=0.;}
    }
                ...
                for ( int j = 0; j < 2; j++) {
                    if (absdepth[j]<WATT)
                        if(h[j]<h[(j+1)%2])
                            ...
                            dhdx = (absdepth[j]/WATT * h[j] + (1.-absdepth[j]/WATT)* h[(j+1)%2]) * koeffmat[j][1] + h[(j+1)%2] * koeffmat[(j+1)%2][1];
                }
            else
...
//        double operatornorm = Math.abs(u_mean)+Math.sqrt(G*Math.max(WATT,absdepth_mean));
        double operatornorm = Math.sqrt(u_mean*u_mean+G*Math.max(WATT,absdepth_mean)); // Spectral Norm, since 3.15.5
	...
	double a_opt = 1.;
        ...
        for ( int j = 0; j < 2; j++) {
            ...
            cureq2_mean += 1./2. * ( cmd.dudt + G * dhdx + cmd.u * dudx
                        + G / Math.pow(Math.max(absdepth[j],WATT), 1./3.) * Math.abs(u[j]) / 42. / 42. / Math.max(absdepth[j],WATT) * u[j] );
        }
        ...
        for (int j=0;j<2;j++){
            ...
            for (int l=0;l<2;l++){
                ...
                cmd.ru -= vorfaktor * ( G * dhdx + u[l] * dudx + 2.* ast*dudx*koeffmat[j][1]
                       + G / Math.pow(Math.max(absdepth[l],WATT), 1./3.) * Math.abs(u[l]) / 42. / 42. / Math.max(absdepth[l],WATT) * u[l] );
            }
            cmd.dudx += 0.5 * dudx;
        }
        return timeStep;
