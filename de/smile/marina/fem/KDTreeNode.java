...
class KDTreeNode {
	...
	public KDTreeNode(FEDecomposition mesh, int[] objects, de.smile.geom.Rectangle2d bounds, int maxObjectsInLeaf,
			boolean opt_split_mode, int direction_mode, int TREE_TYPE, int rekDepth) {
		this.rekDepth = rekDepth;
		nodes++;

		if ((objects.length < maxObjectsInLeaf) || (rekDepth >= maxRekDepth) || bounds.width < 0.01
				|| bounds.height < 0.01)
	...
	public final void newChildren(FEDecomposition mesh, int[] objects, de.smile.geom.Rectangle2d bounds,
			int maxObjectsInLeaf, boolean opt_split_mode, int direction_mode, int TREE_TYPE) {
		newPointInElementSearchChildren(mesh, objects, bounds, maxObjectsInLeaf, opt_split_mode, direction_mode,
				TREE_TYPE);

	}

	public void newPointInElementSearchChildren(FEDecomposition mesh, int[] objects, de.smile.geom.Rectangle2d bounds,
			int maxObjectsInLeaf, boolean opt_split_mode, int direction_mode, int TREE_TYPE) {
		FElement topo;
		int child1Count = 0;
		int child2Count = 0;
		...
		// unguenstige Aufteilung mit sukzessiver Rekursion bis zur max. Rektiefe
		// verhindern
		int test_max = 5;
		...
		if ((child1Count > objects.length - test_max || child2Count > objects.length - test_max) && rekDepth > 5) {
			 ...
			 * newPointInElementSearchChildren(mesh,objects,bounds,maxObjectsInLeaf,
			 * opt_split_mode,mode,TREE_TYPE);
			 * }
			 * else
			 */
			newLeaf(objects, bounds);
		...
		if (multi_threading && rekDepth == 0) {
			KDTreeNodeThread thread1 = new KDTreeNodeThread(mesh, objects1, bounds1, maxObjectsInLeaf, opt_split_mode,
					KDTreeNode.AUTO_DETECT_SPLIT_DIRECTION, TREE_TYPE, rekDepth + 1);
			KDTreeNodeThread thread2 = new KDTreeNodeThread(mesh, objects2, bounds2, maxObjectsInLeaf, opt_split_mode,
					KDTreeNode.AUTO_DETECT_SPLIT_DIRECTION, TREE_TYPE, rekDepth + 1);
			...
			child1 = new KDTreeNode(mesh, objects1, bounds1, maxObjectsInLeaf, opt_split_mode,
					KDTreeNode.AUTO_DETECT_SPLIT_DIRECTION, TREE_TYPE, rekDepth + 1);
			child2 = new KDTreeNode(mesh, objects2, bounds2, maxObjectsInLeaf, opt_split_mode,
					KDTreeNode.AUTO_DETECT_SPLIT_DIRECTION, TREE_TYPE, rekDepth + 1);
		}
	}
	...
	public int[] searchLeafNumbers(Point3d p) {
		if (isLeaf()) {
			...
			// System.out.println(p.getNumber()+" p "+p.getX()+" "+split_x+" p "+p.getY()+"
			// "+split_y);
			if (dir_x) {
				if (p.x <= split)
	...
	protected class KDTreeNodeThread extends Thread {
		...
		protected KDTreeNodeThread(FEDecomposition mesh, int[] objects, de.smile.geom.Rectangle2d bounds,
				int maxObjectsInLeaf, boolean opt_split_mode, int direction_mode, int TREE_TYPE, int rekDepth) {
			this.mesh = mesh;
			this.objects = objects;
			this.bounds = bounds;
		...
		@Override
		public void run() {
			node = new KDTreeNode(mesh, objects, bounds, maxObjectsInLeaf, opt_split_mode, direction_mode, TREE_TYPE,
					rekDepth);
		}
