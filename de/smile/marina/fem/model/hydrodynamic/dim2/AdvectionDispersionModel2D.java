 ...
 */
public class AdvectionDispersionModel2D extends TimeDependentFEApproximation
    ...
    public double ElementApproximation(FElement element) {
        ...
        if (eleCurrentData != null) {
            if (!eleCurrentData.isDry) {
                ...
                AdvectionDispersionModel2DElementData eleADData = AdvectionDispersionModel2DElementData
                        .extract(element);
                ...
                for (int j = 0; j < 3; j++) {
                    ...
                    double result_ADConc_i = -tau_konc * (koeffmat[j][1] * u_mean + koeffmat[j][2] * v_mean)
                            * Koeq1_mean * ele.area;
                    if (result_ADConc_i > 0)
                        result_ADConc_i *= cmd.wlambda; // Konzentration will wachsen, Knoten aber Wattknoten

                    // // tubulenc-term
                    // result_ADConc_i -=
                    // // Diffusionsterm (entspricht ∇⋅(D∇c))
                    // + (koeffmat[j][1] * astx * dsaltconcdx + koeffmat[j][2] * asty * dsaltconcdy)
    ...
    public void timeStep(double dt) {
        ...
        Arrays.stream(fenet.getDOFs()).parallel().forEach(dof -> {
            ...
            if (rIsNaN)
                System.out.println(
                        "AdvectionDispersionTransport is NaN bei " + dof.number + " dconcdt=" + rConcentration);
            resultIsNaN |= rIsNaN;
        });
