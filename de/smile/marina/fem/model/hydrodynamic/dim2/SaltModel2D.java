...
public class SaltModel2D extends TimeDependentFEApproximation implements FEModel, TicadModel, TimeDependentModel {
     ...
     */
    public void initialSolutionFromTicadErgFile(String salterg, int record) throws Exception {
     ...
     * @throws java.lang.Exception
     */
    public void initialSaltConcentrationFromSysDat(String filename, double time) throws Exception {
        this.time = time;
    ...
    public void initialSaltConcentrationFromJanetBin(String filename, double time) throws Exception {
        int anzAttributes = 0;
        double skonc;
        ...
        if (eleCurrentData != null) {
            if (!eleCurrentData.isDry) {
                ...
                for (int j = 0; j < 3; j++) {
                    ...
                    double result_SaltConc_i = -tau_konc * (koeffmat[j][1] * u_mean + koeffmat[j][2] * v_mean)
                            * Koeq1_mean * ele.area;
                    if (result_SaltConc_i > 0)
                        result_SaltConc_i *= cmd.wlambda; // Konzentration will wachsen, Knoten aber Wattknoten
                    // Diffusionsterm (entspricht ∇⋅(D∇c))
                    result_SaltConc_i -= (koeffmat[j][1] * astx * dsaltconcdx + koeffmat[j][2] * asty * dsaltconcdy)
                ...
                for (int ll = 0; ll < 3; ll++) {
                    if (elem.getDOF(ll) == dof) {
                        for (int ii = 1; ii < 3; ii++) {
                            ...
                            double dC = (saltmodeldata.C - tmpdata.C) / 10.
                                    * Math.min(1., tmpcmd.totaldepth / Math.max(CurrentModel2D.WATT, cmd.totaldepth));
    ...
    public ModelData genData(DOF dof) {
        ...
        data.extrapolate = ((data.bsc == null) && ((current.bu instanceof ZeroFunction1d)
                && (current.bv instanceof ZeroFunction1d) && (current.bh == null)));
        return data;
    }
    ...
    public void timeStep(double dt) {
        ...
        Arrays.stream(fenet.getDOFs()).parallel().forEach(dof -> {
            ...
            rSalt *= currentmodeldata.puddleLambda * currentmodeldata.wlambda; // in Pfuetzen und auf trockenen Knoten
                                                                               // keine Veraenderung der Konzentration
            ...
            if (saltmodeldata.C < 0.)
                saltmodeldata.C = 0.;
            /* prevention of concentration grater then max. of 36% = 360 ppt */
            if (saltmodeldata.C > 360.)
                saltmodeldata.C = 360.; // Peter 12.09.19

            boolean rIsNaN = Double.isNaN(rSalt);
            if (rIsNaN)
                System.out.println("Salttransport is NaN bei " + dof.number + " dsaltconcdt=" + rSalt);
            resultIsNaN |= rIsNaN;
        });
